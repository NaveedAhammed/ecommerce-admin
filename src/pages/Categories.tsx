...
const Categories = () => {
  ...
  const handleActionDelete = async (category: CategoryType) => {
    const res = (await axiosPrivate.delete(`/size/delete/${category._id}`))
      .data;
    toast.promise(res, {
        ...
        <TBody>
          {categories?.map((category) => (
            <TRow key={category._id}>
              ...
              <TRowData>
                {dayjs(category?.createdAt?.split("T")[0]).format(
                  "MMM D, YYYY"
                )}
              </TRowData>
              <TableAction
