...
const Sizes = () => {
  ...
  const handleActionDelete = async (size: SizeType) => {
    const res = (await axiosPrivate.delete(`/size/delete/${size._id}`)).data;
    toast.promise(res, {
        ...
        <TBody>
          {sizes?.map((size) => (
            <TRow key={size._id}>
              ...
              <TRowData>
                {dayjs(size?.createdAt?.split("T")[0]).format("MMM D, YYYY")}
              </TRowData>
              <TableAction
